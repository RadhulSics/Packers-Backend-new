...
const searchMoversByName = (req, res) => {
  // let mover=(req.params.moverName).toLowercase()
  console.log("mn",req.params.moverName);
  movers.find({ name: { $regex: req.params.moverName, $options: 'i' },isactive:true })
      .then(services => {
          if (services.length === 0) {
              return res.json({ status:401,message: 'No Movers found with the Name.' });
          }
...
approveOrder,rejectOrder,searchMoversByName}